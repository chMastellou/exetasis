...
for k in range(1,101):
    ...
    while (triliza==False):
        ...
        y=randint(0,2)
        j=randint(1,3)
        #έλεγχος της περίπτωσης που τελειώσουν τα καπάκια ενός είδους πριν γίνει τρίλιζα
        if(Akapakia[j-1]==0):
            continue

        #ελεγχος και αποθήκευση δίσκου

        #αν υπάρχει καπάκι ίδιου ή μεγαλύτερου μεγέθους στην θέση τότε δεν προσμετράται ως έγκυρη προσπάθεια

        if((tetragwno[x][y]==j)or(tetragwno[x][y]>j)):
            continue
        ...
        #αν τα καπάκια είναι λιγότερα από 3 μέσα στο τετράγωνο δεν εχει νόημα η αναζήτηση τρίλιζας
        c=0
        for i in range(0,3):
